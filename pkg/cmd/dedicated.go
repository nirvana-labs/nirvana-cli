...
var rpcNodesDedicatedList = cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[string]{
			Name:      "name",
			Usage:     "Filter by a case-insensitive substring of the RPC Node Dedicated name",
			QueryPath: "name",
		},
		&requestflag.Flag[string]{
			Name:      "network",
		...
		&requestflag.Flag[[]string]{
			Name:      "tag",
			Usage:     "Filter by tags. Repeat the parameter to require several tags; an RPC Node Dedicated must carry all of them.",
			QueryPath: "tags",
		},
		&requestflag.Flag[int64]{
			Name:  "max-items",
