...
var instanceTypesList = cli.Command{
	Name:    "list",
	...
	Action:          handleInstanceTypesList,
...
func handleInstanceTypesList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		_, err = client.InstanceTypes.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, os.Stderr, "instance-types list", obj, format, explicitFormat, transform)
	} else {
		iter := client.InstanceTypes.ListAutoPaging(ctx, params, options...)
		maxItems := int64(-1)
		if cmd.IsSet("max-items") {
			maxItems = cmd.Value("max-items").(int64)
		}
		return ShowJSONIterator(os.Stdout, os.Stderr, "instance-types list", iter, format, explicitFormat, transform, maxItems)
	}
}
...
func handleInstanceTypesGet(ctx context.Context, cmd *cli.Command) error {
	...
	explicitFormat := cmd.Root().IsSet("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, os.Stderr, "instance-types get", obj, format, explicitFormat, transform)
}
