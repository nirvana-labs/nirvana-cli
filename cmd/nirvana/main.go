...
func main() {
	...
	if baseURL, ok := os.LookupEnv("NIRVANA_LABS_BASE_URL"); ok {
		if err := cmd.ValidateBaseURL(baseURL, "NIRVANA_LABS_BASE_URL"); err != nil {
	...
	if err := app.Run(context.Background(), os.Args); err != nil {
		...
		if errors.As(err, &apierr) {
			...
			show_err := cmd.ShowJSON(os.Stdout, os.Stderr, "Error", json, format, app.IsSet("format-error"), app.String("transform-error"))
			if show_err != nil {
